...
namespace MidiPlayerTK
{
    ...
    public enum MPTKCommand : byte
    {
        ...
        ///! @li @c   0 is the lowest bend positions (default is 2 semitones), 
        ///! @li @c   8192 (0x2000) centered value, the sounding notes aren't being transposed up or down,
        ///! @li @c   16383 (0x3FFF) is the highest  pitch bend position (default is 2 semitones)
        /// </summary>
        ...
        ///! @li @c   if MPTKEvent#Meta = SetTempo, MPTKEvent#Value contains new Microseconds Per Quarter Note and MPTKEvent#Duration contains new tempo (quarter per minute).
        ///! @li @c   if MPTKEvent#Meta = TimeSignature, MPTKEvent#Value contains the numerator (number of beats in a bar) and MPTKEvent#Duration contains the Denominator (Beat unit: 1 means 2, 2 means 4 (crochet), 3 means 8 (quaver), 4 means 16, ...)
        ///! @li @c   if MPTKEvent#Meta = KeySignature, MPTKEvent#Value contains the SharpsFlats (number of sharp) and MPTKEvent#Duration contains the MajorMinor flag (0 the scale is major, 1 the scale is minor).
        ///! @li @c   for others, MPTKEvent#Info contains textual information.
        /// </summary>
        MetaEvent = 0xFF,
    }
    ...
    ///! @li @c   MPTKEvent#Controller=RPN_MSB (101) MPTKEvent#Value=0
    ///! @li @c   MPTKEvent#Controller=RPN_LSB (100) MPTKEvent#Value=midi_rpn_event.RPN_PITCH_BEND_RANGE
    ///! @li @c   MPTKEvent#Controller=DATA_ENTRY_MSB (6) MPTKEvent#Value=12
    ///! @li @c   MPTKEvent#Controller=DATA_ENTRY_LSB (38) MPTKEvent#Value=0
