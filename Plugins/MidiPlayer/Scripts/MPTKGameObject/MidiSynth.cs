...
namespace MidiPlayerTK
{
    ...
    public partial class MidiSynth : MonoBehaviour
    {
        ...
        /*private */
        public fluid_synth_status state = fluid_synth_status.FLUID_SYNTH_CLEAN;                /** the synthesizer state -  */ // V2.83 set to private
        ...
        public string MPTK_Channel(int channel)
        {
            string info = "";
            if (MptkChannels != null && channel >= 0 && channel < MptkChannels.Length)
                info = $"Preset:{MPTK_ChannelPresetGetIndex(channel)} Bank:{MPTK_ChannelBankGetIndex(channel)} {MPTK_ChannelPresetGetName(channel)} Note:{MPTK_ChannelNoteCount(channel)} Volume:{MPTK_ChannelVolumeGet(channel)} ";
            return info;
        }
        ...
        /// </summary>
        /// <param name="channel">must be between 0 and 15 or -1 to apply to all channel</param>
        ...
        /// <param name="preset">Preset to force in the range 0 to 127 preset. Set to -1 to restore the Preset and Bank defined in the MIDI file.</param>
        /// <param name="bank">Optionnal. Bank to force in the range 0 to 128. Set to -1 to disable forced bank.</param>
        /// <returns></returns>
        /// 
        /// @code
        /// // Example of changing the forced preset with the Unity GUI:\n
        /// GUILayout.Label($"{midiFilePlayer.MPTK_ChannelForcedPresetGet(channel)}/{midiFilePlayer.MPTK_ChannelBankGetIndex(channel)}");
        /// int forced = (int)GUILayout.HorizontalSlider(midiFilePlayer.MPTK_ChannelForcedPresetGet(channel), -1f, 127f);
        /// if (forced != midiFilePlayer.MPTK_ChannelForcedPresetGet(channel))
        ///    midiFilePlayer.MPTK_ChannelForcedPresetSet(channel, forcedPreset);
        /// @endcode
        /// 
        /// @code
        /// // Another example with MidiExternalPlayer. The forced preset and bank are defined after the MIDI is loaded and before any MIDI message is played.\n
        /// MidiExternalPlayer midiPlayer = FindObjectOfType<MidiExternalPlayer>();
        /// midiPlayer.OnEventStartPlayMidi.AddListener((string v) =>
        ///     {
        ...
        ///// <summary>
        ///// Set forced preset on the channel. MIDI will allways playing with this instrument even if a MIDI Preset Change message is received again.\n
        ///// Set to -1 to disable this behavior.
        ///// </summary>
        ///// <param name="channel">0 to 15 channel</param>
        ///// <param name="preset">0 to 127 preset. Set to -1 to disable forced preset.</param>
        ///// <returns></returns>
        ///// @code
        ///// GUILayout.Label($"{midiFilePlayer.MPTK_ChannelForcedPresetGet(channel)}/{midiFilePlayer.MPTK_ChannelBankGetIndex(channel)}");
        ///// int forced = (int)GUILayout.HorizontalSlider(midiFilePlayer.MPTK_ChannelForcedPresetGet(channel), -1f, 127f);
        ...
        //{
        //    //Debug.Log($"MPTK_ChannelForcedPresetSet preset:{preset} lastprog:{MptkChannels[channel].lastprog} forcedPreset:{MptkChannels[channel].forcedPreset}");
        //    if (CheckParamChannel(channel))
        //    {
        ...
        public bool MPTK_ChannelPresetChange(int channel, int preset, int bank = -1)
        {
            if (CheckParamChannel(channel))
            {
                ...
                //if (sfont.Banks[bank] == null || sfont.Banks[bank].defpresets == null)
                //Debug.LogWarningFormat($"MPTK_ChannelPresetChange: bank {bank} is not defined with sfont {sfont.SoundFontName}");

                //if (preset < 0 || preset >= sfont.Banks[bank].defpresets.Length)
                ...
                //if (sfont.Banks[bank].defpresets[preset] == null)
