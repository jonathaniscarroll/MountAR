...
#endif
                {
                    try
                    {
                        ...
                        if (data != null && data.Length > 4 && System.Text.Encoding.Default.GetString(data, 0, 4) == "RIFF")
                        {
                            ...
                            CreateDefaultSoundFont(sf.SfData, defaultBank, drumBank, synths, restartPlayer, playerToRestart);

                            MidiPlayerGlobal.ImSFCurrent.SoundFontName = Path.GetFileNameWithoutExtension(pathSF);
